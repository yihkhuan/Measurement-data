...
init(name) = sprintf("stats '%s.csv' u 1:2 nooutput;", name) . \
...
sprintf("plot '%s.csv' using ($1*1e-9):2 t 'NA data', '%s.csv' using ($1*3.57e-11):(NaN) axes x2y1 t '', l(x) t 'Lorentzian', lineWidth t 'line width';",name,name)
...
ff(name) = sprintf("stats '%s.csv' u 1:2 nooutput;",name)
