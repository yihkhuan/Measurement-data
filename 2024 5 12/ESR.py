...
currents = np.arange(000,5000) / 1000
...
freq, loss_0 = sp.loss_list(4.800)
for i in tqdm(currents):
    ...
    ## integration
    absorption_int = integrate.cumulative_trapezoid(absorption, freq, initial=0)
    plt.figure()
    plt.ylabel("absorbtion_int, dB")
    plt.xlabel("Frequency, GHz")
    B = sp.current_mag(i,' double-magnet ')
    plt.title("B0 = %.3f mT" % (B))
    # plt.xlim((8.6, 9.8))t
    plt.scatter(freq,absorption_int , marker="+")
    # plt.plot(freq[index - dw:index + dw + 1], absorption_int[index - dw:index + dw + 1])
    plt.savefig("./figs/Int_%.3fA.png" % (i))
