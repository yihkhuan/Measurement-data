...
sprintf("set output './Analyse_data/figs/combined.png';",name)

always = sprintf("set xtics nomirror;") . \
sprintf("set x2tics;") . \
sprintf("set autoscale xfix;") . \
sprintf("set autoscale x2fix;") . \
sprintf("set title 'Change of resonance frequency' font 'Times-New-Roman,22';") . \
...
sprintf("plot '%s.csv' using ($1*1e-9):2 t '%s', '%s.csv' using ($1*3.57e-11):(NaN) axes x2y1 t '';",name,name,name)

reprinting(name) = sprintf("set xrange [A[3] : A[1]];") . \
sprintf("set x2range [A[3] * 0.0357 : A[1] * 0.0357 ];") . \
sprintf("set terminal png size 600,600;") . \
sprintf("replot '%s.csv' using ($1*1e-9):2 t '%s', '%s.csv' using ($1*3.57e-11):(NaN) axes x2y1 t '';",name,name,name)
...
reset
eval init(name [4])
eval get_Values(name [4])
a = A [2]
c = A [2]
j1 = A [2]
j2 = A [2]
eval lfit(name [4])
eval fit_output(name [4])
eval always
eval printing(name [4])
