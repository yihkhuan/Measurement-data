...
def lorentz(x: np.ndarray, Al:float, b:float, a:float, offset:float) -> np.ndarray:
    return Al / np.pi * (b / ((x - a)**2 + b**2)) + offset
...
def calibration_data (resonator_S21, magnet_calibration):
    pass
...
def get_lorentzianBandwidth(parameters, scale):
    '''
    scale: 1 for log scale, 0 for linear scale
    '''
    amplitude, bandwidth, resonanceFreq, offset = parameters
    max = amplitude / np.pi / bandwidth + offset
    if scale < 0 | scale > 1:
        return
    elif scale == 0:
        y = max / 2 #calculating the FWHM of the peak
    else:
        y = max - 3 #deduct 3dB from the peak
    
    difference = np.sqrt((amplitude * bandwidth) / np.pi / (y - offset) - bandwidth ** 2)
    lower_band = resonanceFreq - difference
    upper_band = resonanceFreq +  difference
