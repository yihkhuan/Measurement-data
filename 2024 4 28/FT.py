...
freq, loss0 = sp.loss_list(5.81)
# freq = np.linspace(9,10,10000)

currentlist = sp.current_list(5.84, 6)
out_FT_CSV = np.ones((len(currentlist), 1001, 2))
for current in tqdm(currentlist):
    ...
    output = np.vstack((freq.real * 1e9, np.ones(freq.shape) * current, absorption2))
    output_FT = np.vstack((freq.real * 1e9, np.ones(freq.shape) * current, signal2.real))
    np.savetxt(f"./FT/{current}A.csv",output_FT.transpose(), delimiter=', ', fmt='%.3f')
    
    if sp.find("2024_4_28.csv", "./Absorbtion/"):
        with open("./Absorbtion/2024_4_28.csv", "ab") as outAbsorption:
            outAbsorption.write(b"\n")
            outAbsorption.write(b"\n")
            np.savetxt(outAbsorption, output.transpose(), delimiter=', ', fmt='%.3f', header=str(current))
    else:
        np.savetxt("./Absorbtion/2024_4_28.csv", output.transpose(), delimiter=', ', fmt='%.3f', header=str(current))

    if sp.find("2024_4_28_FT.csv", "./Absorbtion/"):
        with open("./Absorbtion/2024_4_28_FT.csv", "ab") as outAbsorption:
            outAbsorption.write(b"\n")
            outAbsorption.write(b"\n")
            np.savetxt(outAbsorption, output_FT.transpose(), delimiter=', ', fmt='%.3f', header=str(current))
    else:
        np.savetxt("./Absorbtion/2024_4_28_FT.csv", output_FT.transpose(), delimiter=', ', fmt='%.3f', header=str(current))

    output_dict = {}
    output_dict.update({
            current: {
                    "frequency"           : freq.real * 1e9,
                    "absorption"          : absorption2
                    }
                })
    output_FT_dict = {}
    output_FT_dict.update({
            current: {
                    "frequency"           : freq.real * 1e9,
                    "absorption"          : signal2 
                    }
    ...
    description = {
        ...
        "Description": "No teflon, sample in cellophane tape"
    }