...
def lorentzian_fit(freq:np.ndarray, absorption:np.ndarray, freq_expected: float = None):
    ...
    #curve fitting
    dw = 10
    popt,_ = curve_fit(lorentz, freq[index - dw + 1:index + dw + 1], absorption[index - dw + 1:index + dw + 1],
                       p0=(2,.2,max_freq))
    plt.figure()
    plt.plot(freq[index - dw:index + dw], absorption[index - dw:index + dw], marker = "+")
    print(absorption[index])
    Al,b,a,offsetl = popt
    return (Al,b,a,offsetl)

def lorentz(x: np.ndarray, Al:float, b:float, a:float) -> np.ndarray:
