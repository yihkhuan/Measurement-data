...
def esrcontour(magnet_name, loss_0: np.ndarray, current_list, fitting = False, xlim = (0,0), ylim = (0,0)):
    ...
    if fitting == True:
        ...
        for currents, obj in data.items():
            mag = current_mag(float(currents), magnet_name)
        ...
        ax.plot(mag_current(magnetic_field, magnet_name),resonance_freq,"ko", label = "absorption")
        ax.plot(mag_current(mag_pts, magnet_name), freq_pts, "k--", lw=2)
    ...
    if xlim != (0,0):
        ...
        ax.set_xlim(current[0], current[-1])
    if ylim != (0,0):
        ax.set_ylim(ylim)
    else:
    ...
    ax.plot(current[xmax],freq[ymax],"g+", label = "maximum absorption")
    # print(magnet_name)
    ...
    ax.plot(current_pts, np.ones(current_pts.shape[-1]) * resonator_freq, "r--", lw=2, label = "resonance frequency")
...
def get_resonator_params(filenames: list, loss_0, magnet_name):
    ...
    for name in filenames:
        ...
        UTM_2D_plot(freq,l,
            title = f"ESR signal fitting, current = {name} A",
